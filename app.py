...
# -----------------------------------------------------------------------------
# Compatibility helpers for old pickled/joblib models
# -----------------------------------------------------------------------------
try:
...
# Training ranges from the model development dataset.
# They are shown beside the input fields and included in the batch template.
...
def render_logo_or_placeholder(col, logo: Optional[Path], text: str) -> None:
    with col:
        ...
        if not shown:
            # Never emit a broken <img>. If no readable logo exists, show a clean
            # text badge and keep the layout stable.
...
def render_header() -> None:
    ...
    # If no logos are present, the placeholders above keep the header layout clean.
...
def get_training_ranges_from_models(loaded: Dict[str, Dict[str, Any]], feature_names: List[str]) -> Dict[str, Tuple[float, float]]:
    # Keep the displayed ranges tied to the original training data rather than the
    # internal order stored inside individual model objects.
...
# -----------------------------------------------------------------------------
# Batch prediction page and plots
# -----------------------------------------------------------------------------
